...
from psmodule.accounts.models import ExtraInfo

from psmodule.models import ActingRole  # re-exported for ``from psmodule.rbac import ActingRole``
...
def require_extrainfo(user) -> ExtraInfo:
    ...
    # selectors imports from this module.
    from psmodule.selectors import get_extrainfo_for_user

    return get_extrainfo_for_user(user)


def user_is_hod(extrainfo: ExtraInfo) -> bool:
    from psmodule.selectors import is_user_hod

    return is_user_hod(extrainfo)


def user_has_designation(extrainfo: ExtraInfo, name_contains: str) -> bool:
    from psmodule.selectors import has_designation
